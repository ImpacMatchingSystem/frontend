...
export const validateCompanyData = (
  data: any[]
): { valid: ExcelCompanyData[]; errors: string[] } => {
  ...
  data.forEach((row, index) => {
    ...
    if (!row.name || !row.email || !row.password) {
      errors.push(
        `행 ${rowNum}: 필수 필드(name, email, password)가 누락되었습니다.`
      )
      return
    }
...
// 바이어 데이터 검증
export const validateBuyerData = (
  data: any[]
): { valid: ExcelBuyerData[]; errors: string[] } => {
...
export const uploadCompanies = async (
  ...
  for (const companyData of companies) {
    try {
      // 중복 이메일 체크
      const existingCompany = await mockApi.companies.getByEmail(
        companyData.email
      )
      if (existingCompany) {
...
// 바이어 데이터 일괄 업로드
export const uploadBuyers = async (
  buyers: ExcelBuyerData[]
): Promise<{ success: number; errors: string[] }> => {
  let success = 0
  const errors: string[] = []
