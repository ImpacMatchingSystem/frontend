...
export default function BuyerDashboard() {
  ...
  const fetchMyMeetings = async () => {
    ...
    try {
      const [allMeetings, companies] = await Promise.all([
        mockApi.meetings.getAll(),
        mockApi.companies.getAll(),
      ])

      // 현재 바이어의 미팅만 필터링
      const myMeetings = allMeetings.filter(
        meeting => meeting.buyer_id === user.id
      )
  ...
  const stats = {
    ...
    upcoming: meetings.filter(
      m => m.status === 'confirmed' && new Date(m.meeting_time) > new Date()
    ).length,
  }
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="mb-8">
          ...
          <p className="text-gray-600">
            안녕하세요, {user.name}님! 미팅 신청 현황을 확인해보세요.
          </p>
        </div>
                      ...
                      <div className="flex items-center gap-3">
                        <div className="w-12 h-12 bg-gray-100 rounded-lg flex items-center justify-center">
                          {meeting.company.logo_url ? (
                            <img
                              src={
                                meeting.company.logo_url || '/placeholder.svg'
                              }
