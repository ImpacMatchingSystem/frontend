...
export default function CompaniesPage() {
  ...
  const getAvailableTimeSlotsCount = (timeSlots: Company['timeSlots']) => {
    return timeSlots.filter(slot => !slot.isBooked && new Date(slot.startTime) > new Date()).length
  }
        ...
        {filteredCompanies.length === 0 ? (
          ...
          >
            {filteredCompanies.map(company => {
              const availableSlots = getAvailableTimeSlotsCount(company.timeSlots)
                    ...
                    <div className="flex flex-col sm:flex-row gap-2">
                      ...
                      <div className="flex gap-2">
                        <Button variant="outline" size="sm" asChild>
                          <Link href={`/dashboard/buyer/companies/${company.id}`}>
                            상세보기
                          </Link>
                        </Button>
                        {websiteUrl && (
          ...
          <div className="mt-12 text-center">
            <div className="bg-white rounded-lg shadow-sm border p-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm text-gray-500">
                <div>
                  <strong className="text-gray-700">1단계:</strong> 기업 선택 및 상세 정보 확인
                </div>
                <div>
                  <strong className="text-gray-700">2단계:</strong> 가능한 시간대에서 미팅 신청
                </div>
                <div>
                  <strong className="text-gray-700">3단계:</strong> 기업 승인 후 미팅 확정
                </div>
              </div>
            </div>
          </div>
