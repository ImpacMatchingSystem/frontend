...
import { Calendar, Clock, RotateCcw, Settings, Plus, Trash2 } from 'lucide-react'
...
export default function CompanySchedulePage() {
  ...
  const createDefaultSchedule = async () => {
    ...
    for (let i = 0; i < 14; i++) {
      ...
      if (date.getDay() >= 1 && date.getDay() <= 5) {
        for (let hour = 9; hour < 18; hour += 2) { // 2시간 간격
  ...
  const clearAllSlots = async () => {
    if (!confirm('모든 시간대를 삭제하시겠습니까? 이 작업은 되돌릴 수 없습니다.')) {
      return
    }
    ...
    try {
      ...
      for (const slot of timeSlots) {
        if (!slot.isBooked) { // 예약되지 않은 시간대만 삭제
  ...
  const availableSlots = timeSlots.filter(slot => !slot.isBooked && new Date(slot.startTime) > new Date())
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          ...
          <Card>
            ...
            <CardContent>
              ...
              <p className="text-xs text-muted-foreground">예약 가능한 시간대</p>
            </CardContent>
          </Card>
        ...
        <Card className="mb-8">
          <CardHeader>
            ...
            <CardDescription>새로운 미팅 가능 시간대를 추가하세요.</CardDescription>
          </CardHeader>
          <CardContent>
        ...
        <Card className="mb-8">
          <CardHeader>
            ...
            <CardDescription>자주 사용하는 설정을 빠르게 적용하세요.</CardDescription>
          </CardHeader>
          <CardContent>
                ...
                return (
                  <Card key={dateKey}>
                    <CardHeader>
                      ...
                      <CardDescription>
                        {slots.length}개 시간대 • 
                        예약 가능: {slots.filter(s => !s.isBooked && new Date(s.startTime) > new Date()).length}개
                      </CardDescription>
                    </CardHeader>
                    <CardContent>
