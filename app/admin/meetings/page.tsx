      ...
      switch (dateFilter) {
        case 'today':
          filtered = filtered.filter(
            meeting =>
              new Date(meeting.meeting_time).toDateString() === todayStr
          )
          break
        case 'week':
          ...
          filtered = filtered.filter(
            meeting =>
              new Date(meeting.meeting_time) >= today &&
              new Date(meeting.meeting_time) <= weekFromNow
          )
          break
        case 'past':
          filtered = filtered.filter(
            meeting => new Date(meeting.meeting_time) < today
          )
          break
      }
    }
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm text-gray-600">
                          <div>
                            ...
                            {meeting.buyer.company_name && (
                              <p>소속: {meeting.buyer.company_name}</p>
                            )}
                          </div>
                          <div>
                            <p>
